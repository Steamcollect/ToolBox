...
public class IconBrowserWindow : EditorWindow
{
    ...
    private void OnGUI()
    {
        EditorGUILayout.LabelField("Icônes Unity (tooltip = nom exact, clic = copie le nom)", EditorStyles.boldLabel);
        EditorGUILayout.Space();

        float totalWidth = position.width - 20f; // marges
        int columns = Mathf.Max(1, Mathf.FloorToInt(totalWidth / (IconSize + Padding)));
        ...
        while (i < IconNames.Length)
        {
            ...
            for (int c = 0; c < columns && i < IconNames.Length; c++, i++)
            {
                DrawIconButton(IconNames[i]);
            }
    ...
    private void DrawIconButton(string iconName)
    {
        ...
        {
            // On s'assure que le tooltip contient bien le nom exact
        ...
        GUIStyle style = new GUIStyle(GUI.skin.button)
        {
            ...
            margin = new RectOffset((int)Padding, (int)Padding, (int)Padding, (int)Padding)
        };

        if (GUILayout.Button(content, style))
        {
            // Bonus : copie le nom dans le presse-papiers
